...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Deck of Cards');
}
...
async function newDeck(count: number) {
  const res = await pwFetch(`${BASE_URL}/new/shuffle/?deck_count=${count}`);
  if (!res.ok) throw await httpError(res, 'Deck of Cards API error');
...
async function drawCards(deckId: string, count: number) {
  const res = await pwFetch(`${BASE_URL}/${encodeURIComponent(deckId)}/draw/?count=${count}`);
  if (!res.ok) throw await httpError(res, 'Deck of Cards API error');
...
async function shuffleDeck(deckId: string) {
  const res = await pwFetch(`${BASE_URL}/${encodeURIComponent(deckId)}/shuffle/`);
  if (!res.ok) throw await httpError(res, 'Deck of Cards API error');
