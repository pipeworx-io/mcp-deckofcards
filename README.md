...
`tools/list` at `https://gateway.pipeworx.io/deckofcards/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/new_deck \
  -H 'Content-Type: application/json' \
  -d '{"count":1}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/new_deck`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "deckofcards": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-deckofcards"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-deckofcards
```
